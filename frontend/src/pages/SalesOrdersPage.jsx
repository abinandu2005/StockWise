...
import { Plus, Search, MoreHorizontal, Eye, Truck, Package as PkgIcon, ClipboardList, Trash2, CheckCircle2, FileDown, Clock, DollarSign } from "lucide-react";
...
function generateInvoicePDF(order) {
  ...
  doc.text("STOCKWISE", 14, 18);
  doc.setFontSize(9);
  doc.setFont("helvetica", "normal");
  doc.text("Inventory Management System", 14, 26);
  doc.text("stockwise@inventorysystem.com", 14, 33);

  // Invoice title
  doc.setTextColor(255, 255, 255);
  doc.setFontSize(14);
  doc.setFont("helvetica", "bold");
  doc.text("SALES INVOICE", pageW - 14, 18, { align: "right" });
  doc.setFontSize(9);
  doc.setFont("helvetica", "normal");
  doc.text(order.orderNumber, pageW - 14, 26, { align: "right" });
  doc.text(new Date(order.orderDate).toLocaleDateString("en-IN"), pageW - 14, 33, { align: "right" });
  ...
  if (order.customerEmail) doc.text(order.customerEmail, 14, 65);

  // Order info right side
  doc.setFont("helvetica", "bold");
  doc.text("Status:", pageW - 70, 52);
  doc.setFont("helvetica", "normal");
  doc.text(order.status.toUpperCase(), pageW - 14, 52, { align: "right" });
  if (order.trackingNumber) {
    doc.setFont("helvetica", "bold");
    doc.text("Tracking:", pageW - 70, 59);
    doc.setFont("helvetica", "normal");
    doc.text(order.trackingNumber, pageW - 14, 59, { align: "right" });
  }
  ...
  doc.setFontSize(10);
  doc.setFont("helvetica", "bold");
  doc.text("Grand Total:", pageW - 60, finalY);
  doc.setTextColor(37, 99, 235);
  doc.text(`\u20B9${order.totalAmount.toFixed(2)}`, pageW - 14, finalY, { align: "right" });
  ...
  if (order.notes) {
    ...
    doc.setFont("helvetica", "normal");
    doc.text(order.notes, 14, finalY + 6, { maxWidth: 120 });
  }
  ...
  doc.setFont("helvetica", "normal");
  doc.text("Generated by StockWise • Thank you for your business!", pageW / 2, footerY, { align: "center" });

  doc.save(`Invoice-${order.orderNumber}.pdf`);
}

export default function SalesOrdersPage() {
  const { salesOrders, addSalesOrder, updateSalesOrder, deleteSalesOrder, products, updateProduct, markSalesOrderShipped, fetchSalesOrders, fetchProducts } = useData();

  useEffect(() => {
    fetchSalesOrders();
    fetchProducts();
  }, [fetchSalesOrders, fetchProducts]);
  const { user } = useAuth();
  ...
  const handleCreateOrderSubmit = async () => {
    ...
    const newSO = {
      ...
      orderDate: new Date().toISOString().substring(0, 19),
      items: orderItems,
    ...
    // DataContext handles the API call, stock deduction (via backend Feign), and product refresh
    await addSalesOrder(newSO);
    setCreateOpen(false);
  };

  const handleMarkShipped = async (id, orderNumber) => {
    ...
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-4">
        {[
          ...
          { label: "Pending Reserve", value: salesOrders.filter(o => o.status === "pending" || o.status === "processing").length, icon: Clock },
          { label: "Shipped Outflows", value: salesOrders.filter(o => o.status === "shipped").length, icon: Truck },
          { label: "Revenue generated", value: formatCurrency(salesOrders.reduce((a, o) => a + o.totalAmount, 0)), icon: DollarSign },
        ].map((s, i) => {
          const Icon = s.icon;
          return (
            <Card key={i}>
                  ...
                  <td className="px-4 py-3">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuItem onClick={() => generateInvoicePDF(o)}>
          ...
          {viewOrder && (
            <>
              ...
              <div className="flex justify-end pt-2">
                <button
                  onClick={() => generateInvoicePDF(viewOrder)}
      ...
      <Dialog open={deleteConfirmOpen} onOpenChange={setDeleteConfirmOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Cancel and Delete Sales Order</DialogTitle>
            <DialogDescription>
              Are you sure you want to remove and cancel sales order <span className="font-semibold text-foreground">{orderToDelete?.orderNumber}</span>? This action cannot be undone.
            </DialogDescription>
          </DialogHeader>

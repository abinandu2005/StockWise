...
export default function StockAlertsPage() {
  const { products, warehouses, addNotification } = useData();
  const { user } = useAuth();
  ...
  // Alert manager about a specific low-stock product
  const handleAlertManager = (productId: string) => {
    const p = products.find((pr) => pr.id === productId);
    if (!p) return;
    const minLevel = p.minStockLevel || p.minStock || 0;
    const deficit = Math.max(0, minLevel - p.quantity);

    // Save restock request to localStorage
    const existingRequests = JSON.parse(localStorage.getItem("stockwise-restock-requests") || "[]");
    const alreadySent = existingRequests.find((r: any) => r.productId === productId && r.status === "pending");
    if (alreadySent) {
    ...
    localStorage.setItem("stockwise-restock-requests", JSON.stringify([newRequest, ...existingRequests]));
  ...
  const handleAlertAllToManager = () => {
    ...
    alertProducts.forEach((p) => {
      const alreadySent = existingRequests.find((r: any) => r.productId === p.id && r.status === "pending");
    ...
    localStorage.setItem("stockwise-restock-requests", JSON.stringify(existingRequests));

    // Create a single summary notification via DataContext
    addNotification(
      "Bulk Restock Alert from Staff",
      `${user?.name} flagged ${newCount} product(s) for restocking. Please review and create purchase orders.`,
      "warning"
    );

    toast.success(`Manager notified about ${newCount} products needing restock.`);
  };
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {Object.entries(p.warehouseStock || {}).map(([wh, qty]) => (
                            ...
                            >
                              ...
                              {wh}: <span className="font-medium">{qty as number}</span>
                            </span>
                          ))}
                        </div>
                      </td>
